...
/**
 * @brief Sends an RDM GET DMX personality request and reads the response, if
 * any.
 *
 * GET DMX personality requests are sent without parameter data. If a
 * response is received, the response parameter data will include the
 * currently active personality number and the total number of personalities
 * supported by the responding device.
 *
 * Added 2026-08-17: a prior comment on rdm_send_set_dmx_personality() below
 * argued this wasn't needed since GET DEVICE_INFO already reports the
 * current personality - true for read/display purposes, but real-hardware
 * testing (ClayPaky K10) found that this dedicated, minimal request can get
 * a response from a device that's still too busy after a personality change
 * to answer the much larger GET DEVICE_INFO request. Prefer this over GET
 * DEVICE_INFO specifically when verifying a personality SET that got no
 * response.
 *
 * @param dmx_num The DMX port number.
 * @param[in] dest_uid A pointer to the UID of the destination.
 * @param sub_device The sub-device number of the destination.
 * @param[out] personality A pointer to a parameter which will be received in
 * the response.
 * @param[out] ack A pointer to an ACK struct which contains information about
 * the response, including information if no response is received.
 * @return The number of bytes that were received in the response parameter
...
/**
 * @brief Sends an RDM SET DMX personality request and reads the response, if
 * any.
 *
 * SET DMX personality requests are sent with an 8-bit personality number
 * (1-indexed, must be greater than 0) indicating to which personality the
 * responding device(s) should switch. Responding devices send a response
 * without any parameter data. See rdm_send_get_dmx_personality() above for
 * the dedicated GET counterpart - added after real hardware showed the
 * aggregate GET DEVICE_INFO (which also reports current personality) isn't
 * a safe substitute for verifying this specific SET.
 *
 * @param dmx_num The DMX port number.
 * @param[in] dest_uid A pointer to the UID of the destination.
 * @param sub_device The sub-device number of the destination.
 * @param personality_num The personality number to switch to. Must be
 * greater than 0.
 * @param[out] ack A pointer to an ACK struct which contains information about
 * the response, including information if no response is received.
 * @return true if a properly formatted RDM_RESPONSE_TYPE_ACK was received.
